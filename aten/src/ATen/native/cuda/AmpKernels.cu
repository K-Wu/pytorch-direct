                                                     ...
                                                     const Tensor& inv_scale)
{
  ...
  TORCH_CHECK(inv_scale.is_cuda_or_unified(), "inv_scale must be a CUDA tensor.");
  TORCH_CHECK(found_inf.is_cuda_or_unified(), "found_inf must be a CUDA tensor.");
  ...
  if (can_use_fast_route(scaled_grads)) {
    ...
    TORCH_CHECK(scaled_grads[0].is_cuda_or_unified(), "scaled_grads must be CUDA tensors.");
    ...
    for (const Tensor& t : scaled_grads) {
      ...
      TORCH_CHECK(t.is_cuda_or_unified(), "one of scaled_grads was not a CUDA tensor.");
...
{
  TORCH_CHECK(growth_tracker.is_cuda_or_unified(), "growth_tracker must be a CUDA tensor.");
  TORCH_CHECK(current_scale.is_cuda_or_unified(), "current_scale must be a CUDA tensor.");
  TORCH_CHECK(found_inf.is_cuda_or_unified(), "found_inf must be a CUDA tensor.");
