...
using stream_set = std::unordered_set<c10::cuda::CUDAStream>;
...
struct Block {
  ...
  Block(size_t size, int device = 0, cudaStream_t stream = 0)
      : device(device),
        stream(stream),
        stream_uses(),
        size(size),
...
class UVMDeviceCachingAllocator {
 private:
 ...
 public:
  ...
  Block* malloc(size_t size) {
    ...
    AllocParams params(size, &pool, alloc_size, stats);
    ...
    TORCH_INTERNAL_ASSERT(
        (!block_found && params.err != cudaSuccess) || params.block);
    if (!block_found) {
    ...
    c10::reportMemoryUsageToProfiler(
        block, block->size, c10::Device(c10::DeviceType::Unified));
 ...
 private:
  ...
  bool alloc_block(AllocParams& p, bool isRetry) {
    ...
    p.block = new Block(size, p.pool, (char*)ptr);
...
class UVMTHCCachingAllocator {
 ...
 public:
  UVMTHCCachingAllocator() {
    init();
  }

  std::unique_ptr<UVMDeviceCachingAllocator> device_allocator;
  ...
  void init() {
    device_allocator = std::unique_ptr<UVMDeviceCachingAllocator>(
        new UVMDeviceCachingAllocator());
  }

  /** allocates a block which is safe to use from the provided stream */
  void malloc(void** devPtr, size_t size) {
    Block* block = device_allocator->malloc(size);
  ...
  void setMemoryFraction(double fraction, int device) {
    ...
    device_allocator->setMemoryFraction(fraction);
  }

  void emptyCache() {
    device_allocator->emptyCache();
  }
  ...
  void* getBaseAllocation(void* ptr, size_t* outSize) {
    ...
    return device_allocator->getBaseAllocation(block, outSize);
  }

  std::vector<SegmentInfo> snapshot() {
    std::vector<SegmentInfo> result;
    return device_allocator->snapshot();
  }
};

static UVMTHCCachingAllocator caching_allocator;
;
...
struct THCUVMCachingAllocator : public at::Allocator {
  ...
  at::DataPtr allocate(size_t size) const override {
    ...
    if (size != 0) {
      caching_allocator.malloc(&r, size);
    }
    return {r, r, &raw_delete, at::DeviceType::Unified};
  }
  ...
  void cacheInfo(int dev_id, size_t* cachedAndFree, size_t* largestBlock) {
    caching_allocator.device_allocator->cacheInfo(cachedAndFree, largestBlock);
  }
  ...
  void* raw_alloc(size_t nbytes) {
    ...
    C10_CUDA_CHECK(cudaGetDevice(&device));
    void* r = nullptr;
    caching_allocator.malloc(&r, nbytes);
    return r;
  }
...
static THCUVMCachingAllocator thc_uvm_allocator;
void THCUVMAllocator_emptyCache(void) {
  thc_uvm_allocator.emptyCache();
}
at::Allocator* getTHCUVMAllocator(void) {
  return &thc_uvm_allocator;
}